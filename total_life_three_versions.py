...
for i in range(len(trans_filenames)):
    for chunk in t_df[trans_filenames[i]]:
        data_sum = chunk.groupby(['i']).agg({'A_if':'sum'})
        data_new = data_new.append(data_sum)
        data_new = data_new.groupby(['i']).agg({'A_if':'sum'})


# data_new_fv_v1.rename(columns={'A_if':'sum_A_if'}, inplace = True)
# data_new_fv_v1 = data_new_fv_v1.reset_index()

data_new.rename(columns={'A_if':'sum_A_if'}, inplace = True)
...
merged_result = pd.merge(states_df, data_new, on='i',how="left")
...
# lifetime_avg_v3.loc[lifetime_avg_v3.index ==0,"lifetime_i"] = np.inf
...
merge1 = pd.merge(lifetime_avg_v1,lifetime_avg_v2,on=labels,how="outer")
merge2 = pd.merge(merge1,lifetime_avg_v3,on=labels,how="outer")
name1 = 'lifetime_v1,J='+ str(Jmin)
name3 = 'lifetime_v3,J='+ str(J_argmax)
merge2.rename(columns={'lifetime_i_x':name1,'lifetime_i_y':'lifetime_v2','lifetime_i':name3}, inplace = True)

v_state = merge2[labels]
v_state_vec = np.array(v_state)
v_state_vec = v_state_vec.tolist()
merge2.insert(0,'v_state',v_state_vec)
merge2.drop(labels, axis=1, inplace=True)
merge2.rename(columns={'v_state': str(labels)}, inplace = True)

merge2.to_csv('./compare_result/'+molecule+"_lifetime_compare.csv")
