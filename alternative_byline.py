...
data_new["i"] =  list(i_dic.keys())
data_new["sum_A_if"] =  list(i_dic.values())
...
data_new["i"] = [np.float(x) for x in data_new["i"]]
...
lifetime_avg_v3.to_csv('./v3_result/'+molecule+"_v3_byline_"+date+".csv",index = 0)

end_time = time.time()
duration_ms = (end_time - start_time)/60
print('total time used is:',duration_ms,'min')

info_dic = {'molecule':[molecule],'labels':str(labels),'date':date,'time (min)':duration_ms,'J':J_argmax,"n_trans":len(trans_filenames),"algorithm":"byline"}
